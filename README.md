...
### 選配:Echon gateway(圖片 + 影片,不用各家 key)

`om-ext/tools/` 附了兩個接 Echon gateway 的生成工具(`setup.sh` 會自動裝進 OpenMontage):
`echon_gemini_image`(Gemini 圖片)、`echon_gemini_video`(Gemini/Veo 影片,async 建任務+輪詢)。
在 `OpenMontage/.env` 填:
...
填好後這兩個工具就變 AVAILABLE,agent 的供應商選擇引擎會自動把它們當圖片/影片來源。
