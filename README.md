...
### 選配:Echon gateway(圖片 / 影片 / 音樂 / 旁白 / SFX,不用各家 key)

`om-ext/tools/` 附了一整套接 Echon gateway 統一 `/aigc` API 的生成工具(`setup.sh` 會自動裝進 OpenMontage):

| 工具 | 能力 | 端點 |
|---|---|---|
| `echon_image` | 圖片(gpt-image-2,支援 reference_images) | `/aigc/image/generate` |
| `echon_video` | 影片(seedance,t2v / i2v via first_frame) | `/aigc/video/generate` |
| `echon_music` | 背景音樂 | `/aigc/audio/music` |
| `echon_voiceover` | 旁白 / TTS | `/aigc/audio/voiceover` |
| `echon_sfx` | 音效 | `/aigc/sfx/generate` |

全部走「submit → 輪詢 `/tasks/{id}` → 下載」模式(共用 `tools/_echon_gateway.py`)。在 `OpenMontage/.env` 填:

```
ECHON_GATEWAY_URL=<gateway base url .../studio/api/gateway>
ECHON_GATEWAY_TOKEN=<bearer token>
```

填好後這 5 個工具就變 AVAILABLE,agent 的供應商選擇引擎會自動選用。
⚠️ token 是密鑰,只放 `.env`(已 gitignore),**別 commit**。
